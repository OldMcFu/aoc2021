...
for i in range(min, max+1):
    print("index" + str(i))
    distances = numpy.abs(numpy.array(crab_positions) - i)
    fuel_consumption_per_step = [numpy.sum(numpy.arange(0, distance+1)) for distance in distances]
    fuel_consumption = sum(fuel_consumption_per_step)
